...
const boardSlice = createSlice({
  ...
  reducers: {
    loadBoard(state, action: PayloadAction<{ figures: FigureState[] }>) {
      state.figures = action.payload.figures.reduce((obj, cur) => ({ ...obj, [cur.id]: cur }), {})
    },
    loadBoardFigures(state, action: PayloadAction<FigureState[]>) {
      state.figures = action.payload.reduce((obj, cur) => ({ ...obj, [cur.id]: cur }), {})
    },
    figureNewPosition(
      state,
      action: PayloadAction<{ figure: FigureState; position: PositionState }>,
    ) {
      state.figures[action.payload.figure.id].position = action.payload.position
    },
